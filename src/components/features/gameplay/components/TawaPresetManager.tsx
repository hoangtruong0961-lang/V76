...
import { isCotModule } from "../../../../utils/regex";
          ...
          <div className="flex-1 bg-slate-900 p-4 overflow-y-auto">
            ...
            {activeTab === "prompts" && (
              <div className="flex flex-col gap-6 max-w-3xl">
                ...
                {(() => {
                  ...
                  const cotModules = modules
                    .map((m, idx) => ({ ...m, originalIdx: idx }))
                    .filter((m) => isCotModule(m));

                  if (cotModules.length === 0) return null;

                  return (
                    <ConfigSection
                      title="⚙️ Tuyển Tập Chain of Thought (CoT)"
                      description="Truy cập nhanh để bật/tắt các thiết lập CoT Tháp Ngà tối cao tự động được nhận dạng nằm trong danh sách Modules Prompt."
                    >
                      <div className="flex flex-col gap-3">
                        {cotModules.map((mod, index) => (
                          <div
                            key={mod.identifier || mod.originalIdx}
                            className="flex items-center justify-between p-3 bg-stone-50 dark:bg-slate-900/40 rounded-lg border border-stone-200/80 dark:border-slate-800/80 hover:border-stone-300 dark:hover:border-slate-800 transition-all gap-2"
                          >
                            <div className="flex flex-col gap-0.5 pr-2 overflow-hidden flex-1">
                              <span className="text-sm font-bold text-stone-800 dark:text-slate-200 flex items-center gap-2">
                                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${mod.enabled ? "bg-purple-500 animate-pulse" : "bg-stone-400 dark:bg-slate-700"}`}></span>
                                <span className="truncate">{mod.name || mod.identifier || "Mô-đun CoT chưa đặt tên"}</span>
                              </span>
                              <span className="text-xs text-stone-500 dark:text-slate-400 font-normal line-clamp-1 truncate">
                                {mod.content ? mod.content.substring(0, 150) + (mod.content.length > 150 ? "..." : "") : "Không có nội dung."}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              <div className="flex space-x-1">
                                <button
                                  ...
                                  disabled={index === 0}
                                  onClick={() => {
                                    const newMods = [...(config.modules || [])];
                                    const prevIdx = cotModules[index - 1].originalIdx;
                                    const currIdx = mod.originalIdx;
                                    const temp = newMods[prevIdx];
                                    newMods[prevIdx] = newMods[currIdx];
                                    newMods[currIdx] = temp;
                                    updateConfig((c) => ({ ...c, modules: newMods }));
                                  }}
                                  className="p-1.5 sm:p-2 bg-stone-200/50 dark:bg-slate-800/50 rounded-lg text-stone-600 hover:text-stone-900 hover:bg-stone-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700 transition duration-200 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                                  title="Ưu tiên lên trên"
                                >
                                ...
                                <button
                                  type="button"
                                  disabled={index === cotModules.length - 1}
                                  onClick={() => {
                                    const newMods = [...(config.modules || [])];
                                    const nextIdx = cotModules[index + 1].originalIdx;
                                    const currIdx = mod.originalIdx;
                                    const temp = newMods[nextIdx];
                                    newMods[nextIdx] = newMods[currIdx];
                                    newMods[currIdx] = temp;
                                    updateConfig((c) => ({ ...c, modules: newMods }));
                                  }}
                                  className="p-1.5 sm:p-2 bg-stone-200/50 dark:bg-slate-800/50 rounded-lg text-stone-600 hover:text-stone-900 hover:bg-stone-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-700 transition duration-200 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const newMods = [...(config.modules || [])];
                                  newMods[mod.originalIdx] = {
                                    ...newMods[mod.originalIdx],
                                    enabled: !newMods[mod.originalIdx].enabled,
                                  };
                                  updateConfig((c) => ({ ...c, modules: newMods }));
                                }}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition duration-200 cursor-pointer w-28 text-center ${
                                  mod.enabled
                                    ? "bg-purple-500/10 dark:bg-purple-500/15 border-purple-500/30 dark:border-purple-500/40 text-purple-600 dark:text-purple-300 hover:bg-purple-500/20 dark:hover:bg-purple-500/25 shadow-[0_0_12px_rgba(168,85,247,0.12)]"
                                    : "bg-stone-100 dark:bg-slate-950 border-stone-200 dark:border-slate-800 text-stone-500 dark:text-slate-500 hover:bg-stone-200 dark:hover:bg-slate-900"
                                }`}
                              >
                                {mod.enabled ? "Đang hoạt động" : "Đã tắt"}
                              </button>
                            </div>
                          </div>
                        ))}
            ...
            {activeTab === "settings" && (
              <div className="flex flex-col gap-6 max-w-3xl">
                ...
                <ConfigSection 
                  title="Chain of Thought (CoT) & Model Thinking" 
                  description="Cấu hình cho các mô hình có khả năng suy nghĩ của Gemini 2.x/3.x trở lên (Native Thinking Budget)."
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <NumberField
                      label="Thinking Budget (Bắt buộc >= 1024 tokens)"
                    ...
                    />
                    <div className="flex flex-col gap-1.5 p-3 bg-slate-900/50 rounded-lg border border-slate-800 text-xs text-slate-400">
                      <p className="font-bold text-slate-300 mb-1">Cách hoạt động của CoT:</p>
                      <ul className="list-disc pl-4 space-y-1">
                        <li><strong>0 / Mặc định:</strong> Không kích hoạt Native CoT của Gemini.</li>
                        <li><strong>&gt;= 1024 tokens:</strong> Kích hoạt chế độ Native Thinking của Gemini 2.0 Flash / Pro (Ví dụ: 2048 hoặc 4096).</li>
                        <li><em>Lưu ý:</em> Nội dung suy nghĩ sẽ được trích xuất tự động và hiển thị trong khung <strong>&quot;Tawa bắt đầu suy nghĩ&quot;</strong> tại giao diện chat.</li>
                      </ul>
                    </div>
                  </div>
                </ConfigSection>
