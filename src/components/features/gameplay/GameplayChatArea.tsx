...
const TawaMessageRenderer: React.FC<TawaMessageRendererProps> = React.memo(
  ({
    ...
    const processedContent = useMemo(() => {
      ...
      if (!thinkingContent) {
        // Fallback backward-compatibility support for old logs directly storing RAW text with <thinking> tags
        thinkingContent =
      ...
      if (contentBeautify) {
        content = content.replace(
          ...
          (match, speakerNameRaw, dialogueText, offset, string) => {
            ...
            const isLightMode = s.bg === '#e6ebf4' || s.bg.includes('e8e2d5') || s.bg.includes('debca3');
            
            const dynamicBubbleBg = isPC
              ? (isLightMode ? 'linear-gradient(135deg, #eef5ff, #e0ecff)' : s.convexBg)
              : isNPC
                ? (isLightMode ? 'linear-gradient(135deg, #fffcf5, #fef3c7)' : s.flatBg)
                : s.flatBg;

            const dynamicBubbleBorder = isPC
              ? (isLightMode ? '1px solid rgba(2, 132, 199, 0.2)' : s.border)
              : isNPC
                ? (isLightMode ? '1px solid rgba(217, 119, 6, 0.2)' : s.border)
                : s.border;

            const dynamicBubbleShadow = isPC || isNPC ? s.shadowOuter : s.shadowInner;

            const dynamicBubbleColor = isPC
              ? (isLightMode ? '#0369a1' : s.text)
              : isNPC
                ? (isLightMode ? '#b45309' : s.text)
                : s.text;

            const dynamicTailBg = isPC
              ? (isLightMode ? '#e0ecff' : s.card)
              : isNPC
                ? (isLightMode ? '#fef3c7' : s.card)
                : s.card;
            ...
            return `\n\n<div class="flex items-start gap-3 my-6 animate-in fade-in slide-in-from-left-2 duration-300 ${flexDir}">
                    ...
                    <div class="flex flex-col max-w-[80%] ${alignItem}">
                        ...
                        <div class="p-4 rounded-2xl ${roundedShape} relative group text-base md:text-lg leading-relaxed"
                             style="background: ${dynamicBubbleBg}; border: ${dynamicBubbleBorder}; box-shadow: ${dynamicBubbleShadow}; color: ${dynamicBubbleColor}">
                            <div class="${tailClass}" style="background: ${dynamicTailBg}; border: ${dynamicBubbleBorder};"></div>
                            ${dialogueText}
                        </div>
                    </div>
                </div>\n\n`;
    ...
    if (isStreaming) {
      ...
      const contentStream = cleanRawText(displayText || "");
      ...
      return (
        <div className="flex flex-col gap-1 w-full group" ref={containerRef}>
          <div className="flex items-center gap-3 w-full min-h-[16px]">
            ...
            {effectiveMetadata && (
              <div className="flex flex-wrap items-center gap-2 text-[9.5px] uppercase font-bold text-stone-500 dark:text-slate-400 transition-opacity">
                <span className="flex items-center gap-1 bg-stone-100 dark:bg-slate-900/60 px-1.5 py-0.5 rounded border border-stone-200 dark:border-slate-800/60" title="Preset sử dụng">
                  <Settings size={10} className="text-mystic-accent" />{" "}
                  <span className="max-w-[120px] truncate" title={effectiveMetadata.presetUsed}>
                    {effectiveMetadata.presetUsed}
                  </span>
                </span>

                {effectiveMetadata.cotUsed && effectiveMetadata.cotUsed !== "Không dùng" ? (
                  <span 
                    className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-500/20 shadow-[0_0_8px_rgba(168,85,247,0.1)] hover:brightness-110 transition-all cursor-help" 
                    title={`Chain of Thought: ${effectiveMetadata.cotUsed}`}
                  >
                    <Brain className="animate-pulse text-purple-500 dark:text-purple-400" size={10} />{" "}
                    <span className="font-extrabold tracking-wider">{effectiveMetadata.cotUsed}</span>
                  </span>
                ) : (
                  <span className="flex items-center gap-1 bg-stone-100 dark:bg-slate-900/60 px-1.5 py-0.5 rounded border border-stone-200 dark:border-slate-800/60 text-stone-400 dark:text-slate-600" title="CoT: Không dùng">
                    <Brain size={10} />{" "}
                ...
                <span
                  className="flex items-center gap-1 bg-stone-100 dark:bg-slate-900/60 px-1.5 py-0.5 rounded border border-stone-200 dark:border-slate-800/60"
                  title="World Info Entities"
                >
                  <Database size={10} className="text-cyan-600 dark:text-cyan-400" /> {effectiveMetadata.worldInfoConfig}
                </span>
              </div>
            )}
    ...
    return (
      <div
        className="font-sans text-base md:text-lg [&>p]:mb-3 last:[&>p]:mb-0 leading-[20px] font-normal not-italic"
        style={{ color: s.text }}
        ref={containerRef}
      >
        {turnNumber !== undefined && (
...
export const GameplayChatArea: React.FC<GameplayChatAreaProps> = ({
  ...
  if (!isTavernHelperReady) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-3" style={{ backgroundColor: s.bg }}>
  ...
  return (
    <>
      <div
        ...
        className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-5 space-y-4 w-full" style={{ backgroundColor: s.bg }}
      >
        {displayedMessages.map((msg, idx) => {
          ...
          const swipesThinking = msg.swipesThinking || [msg.thinking];
          const displayThinking = swipesThinking[swipeIndex] || "";
          ...
          return (
            ...
            >
              <div
                className={`relative rounded-2xl p-3 md:p-5 leading-relaxed text-base flex flex-col gap-2 transition-all ${
                  !isModel
                    ? "rounded-tr-none max-w-[90%] md:max-w-[85%]"
                    : "bg-transparent pl-0 w-full"
