...
/**
 * Tự động gắn thêm thẻ mở <thinking> ở đầu chuỗi nếu trong chuỗi có thẻ đóng </thinking> (hoặc các biến thể đóng)
 * nhưng lại bị thiếu thẻ mở tương ứng ở trước nó, hoặc nếu chuỗi bắt đầu trực tiếp bằng các nội dung suy nghĩ
 * khi chưa kịp khai báo thẻ mở <thinking> và sau đó có chứa các thẻ hệ thống chính văn khác.
 */
export const ensureThinkingTagsBalanced = (text: string): string => {
  if (typeof text !== 'string' || !text) return text;

  // Tiêu chuẩn hóa sơ bộ các thẻ đóng để dễ tìm kiếm kiểm tra
  let temp = text;
  // Chuẩn hóa tất cả các biến thể thẻ đóng về </thinking>
  temp = temp.replace(/<\/(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\b[^>]*>/gi, '</thinking>');
  temp = temp.replace(/<\s*\/+\s*(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\s*>/gi, '</thinking>');
  temp = temp.replace(/\\(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)>/gi, '</thinking>');

  // Chuẩn hóa tất cả các biến thể thẻ mở về <thinking>
  temp = temp.replace(/<(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\b[^>]*>/gi, '<thinking>');

  // Kiểm tra xem có </thinking> nhưng thiếu <thinking> trước nó
  const firstCloseIdx = temp.indexOf('</thinking>');
  if (firstCloseIdx !== -1) {
    const firstOpenIdx = temp.indexOf('<thinking>');
    if (firstOpenIdx === -1 || firstOpenIdx > firstCloseIdx) {
      // Thiếu thẻ mở <thinking> trước thẻ đóng </thinking> đầu tiên!
      // Chèn <thinking> ở ngay đầu chuỗi dải văn bản
      return '<thinking>\n' + text;
    }
  }

  // Tình huống phụ trợ: AI gõ nội dung suy nghĩ tự do nhưng hoàn toàn KHÔNG có cả thẻ mở lẫn thẻ đóng,
  // nhưng ở cuối / giữa văn bản bộc lộ thẻ mở khác (như <content>, <story>...) 
  // thì tất cả các dòng ở trước đó chính là suy nghĩ thầm kín (Thinking).
  // KHÔNG tính <branches> hay <choices> ở đây vì có thể trước đó là chính văn không bọc thẻ!
  const contentOpenRegex = /<(content|story)\b[^>]*>/i;
  const contentOpenMatch = temp.match(contentOpenRegex);
  if (contentOpenMatch) {
    const openIdx = temp.indexOf('<thinking>');
    if (openIdx === -1) {
      // Hoàn toàn không có thẻ <thinking> nhưng có <content>, <story>...
      // Chúng ta sẽ tự động chèn <thinking> ở đầu và đóng </thinking> ngay trước thẻ mở kia!
      const insertClosePos = contentOpenMatch.index!;
      const beforeContent = text.substring(0, insertClosePos);
      const afterContent = text.substring(insertClosePos);
      
      // Chỉ tự động bao bọc nếu phần trước content thực sự có chữ có nghĩa (chứ không chỉ trống rỗng)
...
export const sanitizeUnclosedThinkingTags = (text: string): string => {
  ...
  // Đầu tiên, sửa lỗi thiếu thẻ mở nếu tồn tại thẻ đóng ở sau hoặc có cấu trúc bất cân bằng
  const preProcessed = ensureThinkingTagsBalanced(text);
  ...
  let balanced = preProcessed;
  balanced = balanced.replace(/<(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\b[^>]*>/gi, '<thinking>');
  balanced = balanced.replace(/<\/(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\b[^>]*>/gi, '</thinking>');

  // Chuẩn hóa các biến thể đóng bị viết sai hoặc có khoảng trắng như < /thinking> hoặc <\thinking>
  balanced = balanced.replace(/<\s*\/+\s*(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)\s*>/gi, '</thinking>');
  balanced = balanced.replace(/\\(ACTIVE_PRESET_COGNITIVE_FRAMEWORK|thinking|think|thinhking|thought|thoughts)>/gi, '</thinking>');
  ...
  while (true) {
    ...
    // Hướng xử lý tối ưu 2: Sử dụng dòng-qua-dòng quét thông minh để tìm điểm chuyển tiếp sang văn kể/hội thoại truyện độc lập ngôn ngữ.
    // Dọn dẹp lỗi của biểu thức lookahead toàn cục cũ (vốn dễ ngỏm nếu truyện chứa ký tự gạch nối hay hai chấm).
    const linesAfter = afterOpenText.split('\n');
    let transitionLineIdx = -1;
    ...
    for (let i = 0; i < linesAfter.length; i++) {
      ...
      if (!line) {
        accumulatedLength += linesAfter[i].length + 1;
        continue;
      }

      const lowerLine = line.toLowerCase();

      // Điểm bẫy thẻ XML hệ thống đột ngột
      if (line.match(/<(content|story|branches|choices|actions|table_stored|tableEdit|set_time|time_cost|incrementalSummary)/i)) {
        transitionLineIdx = i;
        break;
      }

      // Điểm bẫy mốc biên truyện đột ngột
      if (line.match(/\[(BẮT ĐẦU TRUYỆN|BẮT ĐẦU PHẦN TRUYỆN|START_STORY|START WRITING|WRITING START|STORY START)\]/i)) {
        transitionLineIdx = i;
        break;
      }

      // Phân tích dòng văn câu chuyện (có chứa hội thoại tiếng Việt thông thường, không phải bài phân tích hay log hệ thống)
      const isDialogStart = line.match(/^[“"「『—-]\s*[A-ZÀ-Ỹ]/) || line.startsWith('“') || line.startsWith('「') || line.startsWith('|') || line.startsWith('『');
      
      // Đoạn văn tự sự chính văn: đủ dài, không bắt đầu bằng các dấu bullet / tag / dòng hệ thống, không chứa tên nhãn "Goal:", v.v.
      const isDescriptiveParagraph = line.length > 50 && 
        !line.startsWith('-') && 
        !line.startsWith('*') && 
        !line.startsWith('•') && 
        !line.startsWith('[') && 
        !line.startsWith('#') &&
        !line.match(/^[a-zA-Z0-9_\s]{2,20}:/) && // "Section:", "Goal:"
        !lowerLine.includes('compliance') &&
        !lowerLine.includes('rule') &&
        !lowerLine.includes('hoàn thành') &&
        !lowerLine.includes('tiến hành') &&
        !lowerLine.includes('phân tích');

      // Phải đảm bảo khối suy nghĩ (thinking CoT) đã chạy được một chút ngắn (v.v. >100 ký tự) để tránh nhận diện nhầm ở dòng đầu tiên
      if (accumulatedLength > 100 && (isDialogStart || isDescriptiveParagraph)) {
        transitionLineIdx = i;
        break;
      }
    ...
    if (transitionLineIdx !== -1) {
      // Tái tạo lại chuỗi với thẻ </thinking> được chèn ngay trước dòng truyện đầu tiên
      const linesBefore = linesAfter.slice(0, transitionLineIdx);
      const linesAfterPart = linesAfter.slice(transitionLineIdx);
      
      balanced = balanced.substring(0, contentStartIdx) + 
                 linesBefore.join('\n') + 
                 "\n</thinking>\n" + 
                 linesAfterPart.join('\n');
      
      lastSearchIdx = contentStartIdx + linesBefore.join('\n').length + "\n</thinking>\n".length;
      continue;
    }
...
/**
 * Tách biệt suy nghĩ (thinking) và chính văn (text) từ văn bản thô XML của AI.
 * Sử dụng bộ dọn dẹp, cân bằng thẻ cùng bộ phân tách thông minh dòng truyện kể để không bao giờ bị ẩn nhầm.
 */
export const extractThinkingAndContent = (text: string): { thinking: string; text: string } => {
  if (typeof text !== 'string' || !text) return { thinking: '', text: '' };

  const balancedText = sanitizeUnclosedThinkingTags(text);

  const thinking = extractTagContent(balancedText, 'thinking') || 
                   extractTagContent(balancedText, 'think') || 
                   extractTagContent(balancedText, 'thought') || 
                   extractTagContent(balancedText, 'ACTIVE_PRESET_COGNITIVE_FRAMEWORK') ||
                   extractTagContent(balancedText, 'thinhking') || '';

  const mainText = cleanRawText(text);
  ...
  tagsToRemove.forEach(tag => {
    ...
    while (true) {
        ...
        if (closingIndex !== -1) {
            ...
            // Nếu không tìm thấy thẻ đóng:
            if (['ACTIVE_PRESET_COGNITIVE_FRAMEWORK', 'thinking', 'think', 'thinhking', 'thought', 'thoughts'].includes(tag)) {
                // FALLBACK THÔNG MINH CHO THẺ THINKING CHƯA ĐÓNG:
                // Tìm vị trí của hội thoại hoặc dòng biểu đạt chính văn đầu tiên thay vì xóa toàn bộ.
                const remaining = cleaned.substring(contentStartIndex);
                
                // Trích xuất các dòng và tìm dòng truyện đầu tiên
                const remainingLines = remaining.split('\n');
                let splitLineIdx = -1;
                let foundNarrative = false;
                
                for (let j = 0; j < remainingLines.length; j++) {
                    const l = remainingLines[j].trim();
                    if (!l) {
                        continue;
                    }
                    
                    const isDialog = l.match(/^[“"「『—-]/) || l.startsWith('“') || l.startsWith('「') || l.startsWith('|') || l.startsWith('『');
                    const isNarrativeProse = l.length > 20 && 
                        !l.startsWith('-') && 
                        !l.startsWith('*') && 
                        !l.startsWith('•') && 
                        !l.startsWith('[') && 
                        !l.startsWith('#') &&
                        !l.match(/^[a-zA-Z0-9_\s]{2,20}:/) &&
                        !l.toLowerCase().includes('compliance') &&
                        !l.toLowerCase().includes('rule') &&
                        !l.toLowerCase().includes('phân tích');
                        
                    if (isDialog || isNarrativeProse) {
                        splitLineIdx = j;
                        foundNarrative = true;
                        break;
                    }
                ...
                if (splitLineIdx !== -1) {
                    // Cắt bỏ phần suy nghĩ, chỉ lấy từ dòng chính văn trở đi!
                    const linesAfterPart = remainingLines.slice(splitLineIdx);
            ...
            // Nếu không thể phân tách thông minh hoặc với các thẻ khác, xóa toàn bộ phần còn lại (hữu ích cho streaming)
            cleaned = cleaned.substring(0, startIndex);
            break; 
        }
...
export function isCotModule(mod: any): boolean {
  if (!mod) return false;
  const name = (mod.name || "").toLowerCase();
  const id = (mod.identifier || "").toLowerCase();
  const content = (mod.content || "").toLowerCase();

  const hasCotKeywords = 
    name.includes("cot") || 
    name.includes("thinking") || 
    name.includes("suy nghĩ") || 
    name.includes("tư duy") ||
    id.includes("cot") || 
    id.includes("thinking") || 
    id.includes("suy_nghi");

  const hasThinkingTags = 
    content.includes("<thinking>") || 
    content.includes("</thinking>") ||
    content.includes("<thinking_process>") ||
    content.includes("hướng dẫn suy nghĩ") ||
    content.includes("tự động suy nghĩ") ||
    content.includes("giai đoạn suy nghĩ") ||
    content.includes("suy nghĩ của bạn") ||
    content.includes("quy trình tư duy");

  return hasCotKeywords || hasThinkingTags;
}
