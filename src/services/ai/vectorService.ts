...
export const vectorService = {
    ...
    /**
     * Deletes all vectors for a given saveId/campaignId created on or after the specified timestamp
     */
    async rollbackVectors(saveId: string, timestamp: number): Promise<void> {
        try {
            const allVectors = await dbService.getAllVectors();
            const staleVectors = allVectors.filter(
                v => v.saveId === saveId && v.timestamp && v.timestamp >= timestamp
            );
            if (staleVectors.length > 0) {
                console.log(`[VectorRollback] Rolling back ${staleVectors.length} vectors created on or after timestamp ${timestamp}`);
                for (const v of staleVectors) {
                    await dbService.deleteVector(v.id);
                }
            }
        } catch (err) {
            console.error("[VectorRollback] Error rolling back vectors:", err);
        }
    }
};
