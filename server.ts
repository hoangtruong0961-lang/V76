...
async function startServer() {
  ...
  // Shared secure endpoint token. Defaults to 'ark-v2-client' if not configured via environment.
  const proxySecret = process.env.ARK_PROXY_SECRET || 'ark-v2-client';
  if (!process.env.ARK_PROXY_SECRET && process.env.NODE_ENV === 'production') {
    console.warn(`[SECURITY WARNING] ARK_PROXY_SECRET is not set in production! Falling back to 'ark-v2-client'.`);
  }
  ...
  app.post("/api/ai/proxy", proxyRateLimiter, async (req, res) => {
    ...
    // Validate token: Allow either the configured proxySecret or the default client fallback if origin is authorized
    const isTokenValid = authHeader === proxySecret || (isAuthorizedOrigin && authHeader === 'ark-v2-client');
    if (!isTokenValid) {
